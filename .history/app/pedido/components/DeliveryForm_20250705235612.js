...
export default function DeliveryForm() {
  ...
  const productosSubFiltrados = productos
    .filter((p) =>
      tipoActual === "comida"
    ...
    .filter(
      (p) =>
        ...
        subcategoriasSeleccionadas.includes(p.categoria)
    )
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full">
      ...
      <div className="bg-neutral-900 p-4 rounded-xl space-y-4">
        <SelectorSubCategorias
          onTipoChange={(tipo) => setTipoActual(tipo)}
          onSubcategoriasChange={(subs) => setSubcategoriasSeleccionadas(subs)}
        />
